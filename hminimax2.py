...
def eval_function(state):
    ...
    This function penalises a state evaluation with the manhattan distance between pacman and the nearest food dot, and
    the number of food left to eat. It rewards a state evaluation with the the manhattan distance between pacman
    and the ghost
    ...
    return 5 * pac_ghost_dist - min_pac_food_dist - 10 * num_food
    ...
    if depth > 0:
        ...
        for row in state.getFood():
            for element in row:
                if element:
                    food_pos.append((i, j))
                    dist.append(manhattanDistance(food_pos[x], pac_pos))
                    if dist[x] < min_pac_food_dist:
                        min_pac_food_dist = dist[x]
                        closest_dot_index = x
        ...
        if min_pac_food_dist <= manhattanDistance(ghost_pos, food_pos[closest_dot_index]):
            return True
